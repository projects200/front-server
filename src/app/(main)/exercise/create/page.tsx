...
import { usePostExercise, usePostExercisePictures } from '@/hooks/api/useExerciseApi'
...
export default function Create() {
  ...
  const handleSubmit = async (value: ExerciseRecordReq) => {
    ...
    await Promise.all([mutate(['exercise/range'], value.startedAt.substring(0, 7)), mutate(['exercise/list'], value.startedAt.substring(0, 10))])
  ...
  return (
    <>
      ...
      {celebration && earnedPoints !== null && earnedPoints !== 0 && <Celebration onConfirm={handleCelebrationConfirm} earnedPoints={earnedPoints} />}
    </>
  )
}
