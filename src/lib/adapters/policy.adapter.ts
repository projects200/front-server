...
export function adaptExercisePolicy(dto: ExercisePolicyDto): ExercisePolicy {
  ...
  return {
    ...
    recordValidityPeriod: parseInt(
      policyMap['EXERCISE_RECORD_VALIDITY_PERIOD'] || '0',
      10,
    ),
    recordMaxPerDay: parseInt(
      policyMap['EXERCISE_RECORD_MAX_PER_DAY'] || '0',
      10,
    ),
    daysUntilPenalty: parseInt(
      policyMap['PENALTY_INACTIVITY_THRESHOLD_DAYS'] || '0',
      10,
    ),
    penaltyScore: parseInt(
      policyMap['PENALTY_SCORE_DECREMENT_POINTS'] || '0',
      10,
    ),
  }
}
